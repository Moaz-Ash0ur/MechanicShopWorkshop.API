...
public class CreateWorkOrderCommandHandlerTests(WebAppFactory factory)
{
    ...
    public async Task Handle_WithValidData_ShouldSucceed()
    {
        ...
        Assert.Equal(employee.Id, result.Value.Labor!.Id);
        Assert.Equal(Spot.B, result.Value.Spot);
        Assert.Equal(repairTask.Id, result.Value.RepairTasks[0].Id);
        Assert.Single(result.Value.RepairTasks);
    }


    [Fact]
    public async Task Handel_WhenReaitTaskIdsEmpty_ShouldFail()
    {
        // Arrange
        var customer = CustomerFactory.CreateCustomer().Value;
        var vehicle = customer.Vehicles.First();
