  ...
  //  [Authorize]
    public class RepairTasksController : ApiController
    {
        ...
        public async Task<IActionResult> Get(CancellationToken ct)
        {
          ...
          return result.Match(repairTasks => Ok(repairTasks),Problem);
     ...
     //   [Authorize(Roles = nameof(Role.Manager))]
        ...
        {
            var parts = request.Parts.ConvertAll(p => new CreateRepairTaskPartCommand(p.Name, p.Cost, p.Quantity));

            var command = new CreateRepairTaskCommand(request.Name,request.LaborCost, 
                request.EstimatedDurationInMins is not null ? (RepairDurationInMinutes)request.EstimatedDurationInMins : null, parts);

            var result = await _sender.Send(command, ct);

            return result.Match(response => CreatedAtAction(nameof(GetById), new { repairTaskId = response.Id }, response),Problem);
        }
      ...
      //  [Authorize(Roles = nameof(Role.Manager))]
        ...
        public async Task<IActionResult> Update(Guid repairTaskId, [FromBody] UpdateRepairTaskRequest request, CancellationToken ct)
        {
            ...
            var parts = request.Parts.ConvertAll(p => new UpdateRepairTaskPartCommand(p.PartId, p.Name, p.Cost, p.Quantity));
