...
using MechanicShop.Application.Features.WorkOrders.Command.AssignLabor;
...
using RepairDurationInMinutes = MechanicShop.Domain.RepairTasks.Enums.RepairDurationInMinutes;
using Spot = MechanicShop.Domain.Workorders.Enums.Spot;
using WorkOrderState = MechanicShop.Domain.Workorders.Enums.WorkOrderState;
...
namespace MechanicShop.API.Controllers
{
   ...
   // [Authorize]  
    public class WorkOrdersController : ApiController
    {
        ...
        public async Task<IActionResult> Get([FromQuery] WorkOrderFilterRequest filters, [FromQuery] PageRequest pageRequest ,CancellationToken ct)
        {
            ...
            var query = new GetWorkOrdersQuery
                (
                 pageRequest.Page,pageRequest.PageSize,filters.SearchTerm, filters.SortColumn,filters.SortDirection,
                 filters.State is not null ? (WorkOrderState)(int)filters.State : null ,  filters.VehicleId,
                 filters.LaborId,filters.StartDateFrom,filters.StartDateTo,filters.EndDateFrom,
                 filters.EndDateTo,  filters.Spot is not null ? (Spot)(int)filters.Spot : null
                );
    ...
    //    [Authorize(Policy = "ManagerOnly")]
        ...
        {
            var result = await _sender.Send(new CreateWorkOrderCommand((Spot)(int)request.Spot,
                request.VehicleId , request.StartAtUtc,
                request.RepairTaskIds , request.LaborId),ct);
      ...
      //  [Authorize(Policy = "ManagerOnly")]
        ...
        {
            var result = await _sender.Send(new AssignLaborCommand(workOrderId, Guid.Parse(request.LaborId)), ct);
 ...
 //       [Authorize(Roles = $"{nameof(Role.Manager)},{nameof(Role.Labor)}", Policy = "SelfScopedWorkOrderAccess")]
        ...
        {
            var result = await _sender.Send(new UpdateWorkOrderStateCommand(workOrderId, (WorkOrderState)request.State),ct);
     ...
     //   [Authorize(Roles = nameof(Role.Manager))]
        ...
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        [EndpointSummary("Update reapir tasks for work order.")]
        [EndpointDescription("Updates Reapir Tasks of the specified work order. Only managers can perform this operation.")]
     ...
     //   [Authorize(Policy = "ManagerOnly")]
        ...
        [EndpointDescription("Deletes the specified work order permanently.Only managers can perform this operation.")]
